...
const fiyah = pokémon.filter(p => p.types.includes("fire"));
const multiTypes = pokémon.filter(p => p.types.length > 1);
const names = pokémon.map(p => p.name);
const largeNames = pokémon.filter(p => p.id > 99).map(p => p.name);
const poison = pokémon.filter(p => p.types.includes("poison") && p.types.length == 1).map(p => p.name);
const multiFlying = pokémon.filter(p => p.types[1] === "flying").map(p => p.types[0]);
