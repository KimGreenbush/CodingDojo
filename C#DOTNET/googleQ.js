...
let RightMovement = 0;
let LeftMovement = 0;
let position = RightMovement - LeftMovement;
let counter = 0;
onParachute = true;

function moveLeft(){
    //Move left 1 unit of distance in 1 unit of time.
    if (onParachute == true && position != 0)
    {
        wait();
    }
    else
    {
        LeftMovement -= smoot * hour;
        parachuteCheck();
        return LeftMovement;
    }
...
function parachuteCheck(){
    //Check if on a parachute and return boolean based on results
    // parachute positioning for each Robot would be 0
    // then we need Robot A to find Robot B's position 0
